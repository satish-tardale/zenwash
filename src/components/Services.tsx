...
import { CheckCircle, Clock, Shirt, Sparkles } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 mb-6">
            Choose Your <span className="bg-gradient-to-r from-blue-900 to-cyan-400 bg-clip-text text-transparent">Service Level</span>
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            From everyday essentials to luxury care, we have the perfect service for every need and budget.
          </p>
        </div>
          ...
          {services.map((service, index) => (
            <div
              ...
              className={`relative p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 ${
                service.popular ? 'transform scale-105 border-2 border-cyan-200' : 'border border-gray-200'
              }`}
            >
              {/* Popular Badge */}
              {service.popular && (
              ...
              {/* Service Header */}
              <div className={`bg-gradient-to-r ${service.gradient} ${service.textColor} p-6 rounded-xl mb-6`}>
        ...
        <div className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-2xl p-8">
         ...
         <div className=" ml-32 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto justify-items-center">
  {[
    { icon: <Shirt className="h-6 w-6" />, name: "Dry Cleaning", desc: "Professional dry cleaning" },
    { icon: <Clock className="h-6 w-6" />, name: "Express Service", desc: "Same-day turnaround" },
    { icon: <CheckCircle className="h-6 w-6" />, name: "Stain Removal", desc: "Specialized treatment" }
  ].map((service, index) => (
